...
public class Controller {
    ...
    @PostMapping("/professeurs/{profId}/assign-element")
    public ResponseEntity<String> assignElementToProfessor(@PathVariable Long profId, @RequestParam String elementName) {
        // Fetch the professor
        Optional<Professeur> optionalProfesseur = professeurRepository.findById(profId);
        ...
        Element element = elementRepository.findByCode(elementName);
        if (element == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Element not found");
        }

        // Assign the element to the professor
        element.setProfeseur(professeur);
        elementRepository.save(element);

        return ResponseEntity.ok("Element successfully assigned to the professor");
    }
